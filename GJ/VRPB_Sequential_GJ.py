...
for path in GJ:
    ...
    obj_B = out_B.get()
    y_LB = r_B.get()
    B_head = list(set([i[1] for i in y_LB if i[0]==0]))
    ...
    while iter<=max_iteration:
        if iter%2 == 1:
            ...
            x_LB = r_L.get()
            z_LB = r_C.get()
            L_tail = list(set([i[0] for i in x_LB if i[1]==0]))
...
Solution.to_csv('Solution_Sequential_GJ'+str(datetime.now().strftime('%Y-%m-%d_%H-%M'))+'.csv', index=False)
##############################################################################
