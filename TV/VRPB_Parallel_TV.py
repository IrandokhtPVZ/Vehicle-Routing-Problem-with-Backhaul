...
for path in TV:
    ...
    while iter<=max_iteration:
        ...
        local_LB_without_penalty = objective_value(x_LB, y_LB, z_LB)
        ...
        # solve upper bound problem to obtain feasible solution
...
Solution.to_csv('Solution_Parallel_TV'+str(datetime.now().strftime('%Y-%m-%d_%H-%M'))+'.csv', index=False)
##############################################################################
